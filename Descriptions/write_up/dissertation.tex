...
With reinforcement learning introduced and with it TD-learning, I will now link TD-learning to light transport simulation for importance sampling light path directions. Specifically, due to the benefits discussed of Expected Sarsa over Q-learning and Sarsa, I will represent the rendering equation as an Expected Sarsa learning rule by following the details outlined by Nvidia's work \cite{dahm2017learning}. First, the Expected Sarsa learning rule's summation over the set of all actions $\mathcal{A}$ can be represented as an integral with respect to an action $a$, as shown in Equation \ref{eq:sarsa_integral}, implying the action space is continuous:
...
\noindent
Then, by matching the following terms from the Expected Sarsa learning rule, Equation \ref{eq:match_q_terms} is formed:
\begin{conditions}
...
S_{t+1}   &  The result of the closest intersection of a light ray from $x$ in direction $\omega$, $y = h(x, \omega)$ \\

R_{t+1} & Is now the emitted radiance from point $y$ in direction $-\omega$, $L_e(y, -\omega)$\\

\gamma \cdot \pi(a|S_{t+1}) & Corresponds to the BRDF ($f_s$) and cosine term $cos(\theta_i)$, $f_r(\omega_i, y, \omega) \cdot \cos(\theta_i)$\\
    
Q(S_t, A_t) & Radiance incident on $x$ from direction $\omega$ ($L_i(x, \omega)$), $Q(x, \omega)$\\
\end{conditions}

\begin{equation}
Q(x, \omega) = L_e(y, -\omega)  + \int_\Omega Q(y, \omega_i) \cdot f_r(\omega_i, y, \omega) \cdot \cos(\theta_i) d\omega_i 
\label{eq:match_q_terms}
\end{equation}

To turn Equation \ref{eq:match_q_terms} into a learning rule for path tracing, a simple update rules suffices with learning rate $\alpha$ shown in \ref{eq:expected_sarsa_td_learning}.
