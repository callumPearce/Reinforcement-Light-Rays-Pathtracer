...
Path tracings ability to model light transport faithfully to that of photons in 
...
My work in this thesis focuses on developing importance sampling techniques for
light transport simulation in forward Path tracing. In particular, More specifically,
...
reinforcement learning Path tracer using Importance sampling. Note, many 
physically-based rendering techniques can benefit in a similar way from 
Importance sampling \cite{}, as these techniques share many properties 
of light transport simulation with Path tracing.\\

% Comparison between importance sampling and path tracing with the same number of samples

It is paramount that the modified Path tracing algorithm continues to accurately
simulates global illumination in order to produce photo-realistic images in single
...
illumination approximation made by the Importance sampling methods compared
to that of the naive forward Path tracing algorithm.


\section{Reinforcement Learning for Ray Direction Importance Sampling}
