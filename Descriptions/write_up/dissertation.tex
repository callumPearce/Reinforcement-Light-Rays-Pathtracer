...
Monte Carlo path tracing was originally introduced by Jim Kajiya's original paper on the Rendering Equation \cite{kajiya1986rendering}, but before I get to the rendering equation I must first introduce Monte Carlo Integration. Monte Carlo Integration is a technique to estimate the value of an integral, Equation \ref{eq:integral} represents this integral for the one-dimensional case.
...
The idea behind Monte Carlo integration is to approximate the integral by randomly sampling points to evaluate the integral at, and then averaging the solution to the integral for all the sampled points. More formally, basic Monte Carlo integration approximates a solution to this integral using the numerical solution in Equation \ref{eq:monte_carlo}. Where $\langle F^N \rangle$ is the approximation of $F$ using $N$ samples. However, basic Monte Carlo integration only produce a non-bias estimate when sample points $x^i$ are chosen from a uniform random distribution.

\begin{equation}
\label{eq:monte_carlo}
\langle F^N \rangle = (b - a) \frac{1}{N} \sum^{N-1}_{i=0} f(x^i)
\end{equation}

\begin{equation}
\label{eq:generalized_mc}
\langle F^N \rangle = \frac{1}{N} \sum^{N-1}_{i=0} \frac{f(x^i)}{\frac{1}{(b-a)}} 
 = \frac{1}{N} \sum^{N-1}_{i=0} \frac{f(x^i)}{pdf(x_i)}
\end{equation}

To extend to Generalized Monte Carlo integration where sample point may be sampled from any distribution, the function must be divided by the probability density function evaluated at that sample point as shown in Equation \ref{eq:generalized_mc}. This approach ensures that the Monte Carlo estimate $\langle F^N \rangle$ is unbiased, as areas samples with high probability of being chosen will have a lower weighting in their contribution to the estimate, and areas of lower probability will have a higher contribution weighting \cite{}.

Now Generalized Monte Carlo integration can be used to reduce the variance in the integrals estimated value by sampling from a probability density function which more closely matches the function being integrated $f$ (mb appendix). As samples which highly contribute to the integrals value will be sampled more than those which are not. This is known as importance sampling for Monte Carlo integration.

\begin{itemize}
\item Properties of monte and its advntage
\item link to the rendering equation 
\end{itemize}
