...
\section{Path Tracing and its significance in Computer Graphics}
Path Tracing is a Monte Carlo method for rendering photo-realistic images of 3D scenes by accurately approximating global illumination \cite{christensen2016path}. 

Simulating global illumination means that Path tracing accounts for both direct and indirect illumination. Direct illumination being rays of light emitted from a light source, which reflect off exactly one surface before reaching the camera in the scene. Whereas indirect illumination are ray of light which reflect 2 or times before reaching the camera.

\begin{itemize}
\item Pinhole camera ?
\item Shooting a ray though a pixel (check Carl's lecture slides to find exactly what it is called and find an image rather then describing it in too much detail)
\end{itemize}
