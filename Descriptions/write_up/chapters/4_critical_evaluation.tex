...
Here, a range of techniques are used to evaluate both the Expected Sarsa and the Neural-Q path tracers presented in Chapter \ref{chap:td_deep_sampling} against one another on their ability to reduce noise in rendered images produced by path tracing. However, the trade-off's each algorithm makes and their significance must also be assessed. This includes memory requirements, ability to run in parallel, scaling with scene complexity and current performance bottlenecks.
