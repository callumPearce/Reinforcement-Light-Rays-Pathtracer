...
Following TD-learning, as the optimal value function $q_*$ (or $q_\pi$ in the previous section) is not available, it is possible to instead bootstrap using the current estimate of the value function \cite{deep_rl_function_approx}. Recall that bootstrapping for the value function is when the updated current estimate of the optimal value function for a state-action pair is partially based on new experience data, and the current estimated value of the next selected state-action pair when following policy $\pi$. An example of this is the Expected Sarsa method in section \ref{sec:td_learning}, Equation \ref{eq:expected_sarsa}. Equation \ref{eq:td_error_sarsa} gives what is known as the TD error ($\delta_t$) for Expected Sarsa at time step $t$. It is called an error as it gives the difference between the current estimate $Q(S_t, A_t)$ and the better estimate $R_{t+1} + \gamma \sum_a \pi(a | S_{t+1}) Q(S_{t+1}, a)$ when following policy $\pi$. Notice that the TD error is an estimate that has been made at time step $t$, meaning the error depends on the next state and the next reward which can both change across time steps.

\begin{equation}
\delta_t =\left( R_{t+1} + \gamma \sum_a \pi(a | S_{t+1}) Q(S_{t+1}, a)\right) - Q(S_t, A_t)
\label{eq:td_error_sarsa}
\end{equation}

It is this TD error that can be used to form the loss function for an ANN to approximate the value function, as shown in Equation \ref{eq:loss_expected_sarsa}. The gradient of this loss function can be calculated by Equation \ref{eq:semi_gradient_one_step_exp_sarsa} which is used with an optimizer for updating the parameters $\theta$ during learning. It is known as semi-gradient one-step Expected Sarsa \cite{sutton2011reinforcement}, and is the only deep reinforcement learning rule I shall be using from here on for the Neural-Q path tracer.

\begin{equation}
J(\theta) = \left(R_{t+1} + \gamma \sum_a \pi(a | S_{t+1}) \left[\hat{q_\theta}(S_{t+1}, a) \right] \right) -
\hat{q_\theta}(S_t, A_t)
\label{eq:loss_expected_sarsa}
\end{equation}

\begin{equation}
\triangledown _\theta J(\theta)  = \left( \left(R_{t+1} + \gamma \sum_a \pi(a | S_{t+1}) \left[\hat{q_\theta}(S_{t+1}, a) \right] \right) -
\hat{q_\theta}(S_t, A_t) \right) \triangledown_\theta \hat{q_\theta}(S_t, A_t)
\label{eq:semi_gradient_one_step_exp_sarsa}
\end{equation}

CONVERGENCE??? PG 197 SUTTON

\noindent
Where:
\begin{conditions}
\triangledown_\theta & Gradient w.r.t $\theta$\\
\pi & The current policy followed\\
