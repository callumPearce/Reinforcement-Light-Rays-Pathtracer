...
\SetKwProg{Fn}{Function}{ }{end}
\SetAlgoLined
 \Fn{renderImage(camera, scene, decay)}{  
   $ANN = \text{initANN()}$\\
   \For{$i = 1$ \KwTo $N$}{
       \For{$b = 1$ \KwTo $\text{Batches}$}{
           \For{$k = 1$ \KwTo $\text{BatchSize}$}{
               $p \leftarrow \text{getPixel(j,k)}$\\
               $ray \leftarrow \text{initializeRay(p, camera)}$\\
               \For{$j=1$ \KwTo $\infty$}{
                   $(y, cos(\theta)) \leftarrow \text{closestIntersection}(ray, scene)$\\
                   \If{$j > 1$}{
                       $\hat{q_\theta}x \leftarrow ANN.getQValue(x, \omega)$\\
                       $\hat{q_\theta}y \leftarrow ANN.getMaxQValue(y)$\\
                       $\left( L_e(y, -\omega) + \left[ \max_{\omega_i} \left(\hat{q_\theta}(y, \omega_i) f_s(\omega_i, y, \omega) (\omega_i \cdot \mathbf{n}) \right) \right] \right) - \hat{q_\theta}(x, \omega)$
                   }
                   \If{$\text{noIntersection}(y)$}{
                       $ray.throughput \leftarrow ray.throughput \cdot \text{environmentLightRadiance(ray,y)}$
                   }
                   \If{$\text{areaLightIntersection}(y)$}{
                      $ray.throughput \leftarrow ray.throughput \cdot \text{areaLightRadiance(ray,y)}$
                   }
                   \If{$j > 1$}{
                       $(\omega, \rho_i, f_s) \leftarrow \text{sampleRayDirEtaGreedy}(eta, ray)$\\
                       $ray.throughput \leftarrow ray.throughtput \cdot f_s \cdot cos(\theta) / \rho_i
                       ray \leftarrow (y, \omega)$
                   }
               }
           }
 ...
 \caption{Neural-Q forward path tracer. Samples a single light path trough every pixel in the screen.}
\end{algorithm}
