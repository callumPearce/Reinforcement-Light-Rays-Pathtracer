...
In order to yield smoother sample gradient for training, a minibatching method is used where the gradient information for light paths is accumulated over a small batch of size $BatchSize = 128$ to train the network. This was proven to be succesful for a range of physical control tasks in\cite{lillicrap2015continuous}. While Algorithm \ref{alg:neural_q_pathtracer} iterates through the batches to clearly represent the steps the algorithm takes, in practice all computation with the batch loops are done in parallel. 
