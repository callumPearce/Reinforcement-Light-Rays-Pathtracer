...
void get_radiance_volume_shapes(vector<Surface>& surfaces){
    ...
    RadianceVolume rv = RadianceVolume(vec4(0.f, 0.f, 0.f, 1.f), vec4(0, -1.f, 0, 1.f));
    // Get its vertices
    ...
    for (int x = 0; x < GRID_RESOLUTION; x++){
        for (int y = 0; y < GRID_RESOLUTION; y++){
            ...
            surfaces.push_back(Surface(v1, v2, v3, Material(vec3(0.8f, 0.8f, 0.8f))));
            surfaces.push_back(Surface(v2, v3, v4, Material(vec3(0.8f, 0.8f, 0.8f))));
        }
    }
}